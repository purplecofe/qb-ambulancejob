...
-- Events
RegisterNetEvent('hospital:client:ambulanceAlert', function(coords)
    local data = {displayCode = '10-47', description = '民眾受傷', isImportant = 0, recipientList = {'police', 'bcso', 'sasp', 'saspr', 'ambulance'}, length = 5000, blipSprite = 303, blipColour = 1, blipScale = 1.5}
    local dispatchData = {dispatchData = data, caller = 'Alarm', coords = coords}
    TriggerServerEvent('wf-alerts:svNotify', dispatchData)
end)

RegisterNetEvent('hospital:client:ambulance14A', function(coords)
    local data = {displayCode = '10-14A', description = 'EMS 倒地', isImportant = 1, recipientList = {'police', 'bcso', 'sasp', 'saspr', 'ambulance'}, length = 10000, blipSprite = 303, blipColour = 1, blipScale = 1.5}
    local dispatchData = {dispatchData = data, caller = 'Alarm', coords = coords}
    TriggerServerEvent('wf-alerts:svNotify', dispatchData)
end)

RegisterNetEvent('hospital:client:ambulance14B', function(coords)
    local data = {displayCode = '10-14B', description = 'EMS 倒地', isImportant = 0, recipientList = {'police', 'bcso', 'sasp', 'saspr', 'ambulance'}, length = 10000, blipSprite = 303, blipColour = 1, blipScale = 1.5}
    local dispatchData = {dispatchData = data, caller = 'Alarm', coords = coords}
    TriggerServerEvent('wf-alerts:svNotify', dispatchData)
end)
...
RegisterNetEvent('hospital:client:LayOnBed', function(id)
    TriggerEvent('hospital:client:SendToBed', bedId, Config.Locations["otherBeds"][bedId], isRevive)
    canLeaveBed = true
end)
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            local pos = GetEntityCoords(PlayerPedId())
            exports['ps-ui']:DisplayText(Lang:t('text.bed_out'), 'primary')
...
CreateThread(function()
    for k, v in pairs(Config.Locations["checking"]) do
        exports['qb-target']:AddBoxZone("PillBoxChecking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
            name = "PillBoxChecking"..k,
            heading = -72,
            debugPoly = false,
            minZ = v.z - 2,
            maxZ = v.z + 2,
...
CreateThread(function()
            ...
            options = {
                ...
                },
                {
                    ...
                    action = function(entity)
                        QBCore.Functions.Notify('床位已重置', 'success', 3000)
                        TriggerServerEvent("hospital:server:LeaveBed", k)
                    end,
                    job = "ambulance"
                }
            },
    ...
    for k, v in pairs(Config.Locations["otherBeds"]) do
        exports['qb-target']:AddBoxZone("otherBeds" .. k, v.coords, 2.5, 2.3, {
            name = "otherBeds" .. k,
            heading = -20,
            debugPoly = false,
            minZ = v.coords.z - 1,
            ...
            options = {
                {
                    type = "client",
                    action = function(entity)
                        TriggerEvent('hospital:client:SendToBed', k, Config.Locations["otherBeds"][k], false)
                    end,
                    icon = "fas fa-bed",
                    label = "躺在床上",
                }
